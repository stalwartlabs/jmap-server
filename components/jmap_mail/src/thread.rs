...
use jmap_store::{
    changes::JMAPState, id::JMAPIdSerialize, json::JSONValue, JMAPChangesResponse, JMAPError,
    JMAPGet, JMAPGetResponse, JMAPId, JMAP_MAIL, JMAP_THREAD,
};
use store::{Comparator, DocumentSet, FieldComparator, Filter, Store, Tag};

use crate::{changes::JMAPMailLocalStoreChanges, JMAPMailIdImpl, JMAPMailProperties, MessageField};

pub trait JMAPMailLocalStoreThread<'x>: Store<'x> + JMAPMailLocalStoreChanges<'x> {
    fn thread_get(
        &'x self,
        ...
        if thread_ids.len() > self.get_config().jmap_mail_options.thread_max_results {
