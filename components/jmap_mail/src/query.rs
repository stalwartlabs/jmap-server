...
use jmap_store::{
    local_store::JMAPLocalStore, JMAPFilter, JMAPLogicalOperator, JMAPQuery, JMAP_MAIL,
};
    ...
    T: Store<'x>,
{
    ...
    fn mail_query(
        ...
        let mut result = Vec::with_capacity(if query.limit > 0 {
            query.limit
        } else {
            doc_ids.size_hint().0
        });
        ...
        for doc_id in doc_ids {
            ...
            if query.limit > 0 && result.len() == query.limit {
                break;
            }
        }
